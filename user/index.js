...
router.post('/editUserInfo', (req, res) => {
    ...
    const sql = "UPDATE user SET username = ?, email = ?, phone = ?, address = ?, signature = ? WHERE id = ?";
    db.query(sql, [username, email, phone, address, signature, id], (err, result) => {
        if (err) return res.status(500).send('数据库更新失败！' + err);
        if (result.affectedRows === 0) return res.status(404).send('无用户信息或数据未变化！');
