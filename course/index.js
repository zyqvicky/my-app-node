...
router.get('/getList', (req, res) => {
    ...
    const countSql = `
        ...
        FROM course
        ${category && category !== '全部' ? `WHERE category = '${category}'` : ''}`;
...
router.get('/getTopNew', (req, res) => {
    ...
    db.query(sql, [startDate, startDate], (err, result) => {
        if (err) return res.status(500).send('数据库查询失败！' + err);
        if (result.length < 1) return res.status(404).send('没有找到符合条件的课程！');
...
router.get('/getTopRate', (req, res) => {
    const sql = 'SELECT * FROM course WHERE rate = 5 ORDER BY enrollNum DESC LIMIT 3';
